...
function computeKpis(period: Period, selectedCreators: string[]): KpiSet {
  const ds = KPI_DATASETS[period] ?? KPI_DATASETS["7"];
  const allIds = CREATORS.map((c) => c.id);
  const isAll = selectedCreators.length === 0 || selectedCreators.length === allIds.length;
  const share = isAll ? 1 : selectedCreators.reduce((s, id) => s + (CREATOR_KPI_SHARE[id] ?? 0), 0);
  const scale = (n: number | null) => (n === null ? null : Math.round(n * share));
  return {
    invitations: { value: scale(ds.invitations.value), delta: ds.invitations.delta, spark: ds.invitations.spark.map((v) => Math.round(v * share)) },
    acceptanceRate: { value: ds.acceptanceRate.value, unit: "%", delta: ds.acceptanceRate.delta, spark: ds.acceptanceRate.spark },
    messages: { value: scale(ds.messages.value), delta: ds.messages.delta, spark: ds.messages.spark.map((v) => Math.round(v * share)) },
    meetings: { value: scale(ds.meetings.value), delta: ds.meetings.delta, spark: ds.meetings.spark.map((v) => Math.round(v * share)) },
    calls: { value: scale(ds.calls.value), delta: ds.calls.delta, spark: ds.calls.spark.map((v) => Math.round(v * share)) },
  };
}
...
export function KpiBar({ period, creators }: { period: Period; creators: string[] }) {
  const kpis = useMemo(() => computeKpis(period, creators), [period, creators]);

        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "24px 28px" }}>
          ...
          <Block label="Participants">
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <Participant name={member?.name ?? "Vous"} sub="Andoxa" color={member?.color ?? "#0052D9"} accent={member?.accent ?? "var(--cal2-blue-tint)"} ini={member?.initials ?? "VO"} status="organizer" />
              {event.prospect && (
                <Participant
                  name={event.prospect} sub={event.company} color="var(--cal2-text-soft)" accent={avatarColor(event.prospect)} ini={initials(event.prospect)}
          ...
          <Block label={event.meeting === "inperson" ? "Lieu" : "Visio"}>
            {event.meeting !== "inperson" ? (
              ...
              <div style={{ padding: "12px 16px", background: "var(--cal2-surface-3)", border: "1px solid var(--cal2-border-faint)", borderRadius: 12, fontSize: 13, color: "var(--cal2-text)" }}>

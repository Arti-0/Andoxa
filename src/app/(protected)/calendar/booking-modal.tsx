...
export function BookingModal({ open, onClose }: Props) {
  ...
  return (
    <>
      <div onClick={onClose} className="cal2-root cal2-fade-in" style={{ position: "fixed", inset: 0, background: "var(--cal2-overlay)", backdropFilter: "blur(2px)", zIndex: 200 }} />
      <div
        className="cal2-root cal2-fade-in"
        style={{ position: "fixed", top: "50%", left: "50%", transform: "translate(-50%,-50%)", width: "92%", maxWidth: 640, maxHeight: "88vh", background: "var(--cal2-surface)", borderRadius: 12, zIndex: 201, boxShadow: "var(--cal2-shadow-lg)", display: "flex", flexDirection: "column", overflow: "hidden" }}
      >
        <div style={{ height: 4, background: "var(--cal2-primary)", flexShrink: 0 }} />
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "8px 22px 22px" }}>
          {isLoading ? (
            ...
            <>
                  ...
                  <Field label="WhatsApp post-RDV">
                    <label style={{ display: "flex", alignItems: "flex-start", gap: 10, cursor: "pointer", padding: "10px 12px", border: "1px solid var(--cal2-border-faint)", borderRadius: 8, background: "var(--cal2-surface)" }}>
                      <span
                        ...
                        style={{ position: "relative", width: 32, height: 18, background: showPostBookingWaNotice ? "var(--cal2-primary)" : "var(--cal2-border)", borderRadius: 999, flexShrink: 0, transition: "background 140ms", cursor: "pointer", marginTop: 1 }}
                      >
                    ...
                    {SLOT_OPTIONS.map((m) => {
                      ...
                      return (
                        <button
                          ...
                          style={{ padding: "6px 14px", background: active ? "var(--cal2-blue-tint)" : "var(--cal2-surface)", color: active ? "var(--cal2-primary)" : "var(--cal2-text-muted)", border: `1px solid ${active ? "var(--cal2-primary)" : "var(--cal2-border-soft)"}`, borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}
                        >
                          {m} min
                        </button>
                      );
                ...
                >
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    ...
                    <button
                      ...
                      style={{ alignSelf: "flex-start", padding: "6px 12px", background: "var(--cal2-surface)", color: "var(--cal2-primary)", border: "1px dashed var(--cal2-border-soft)", borderRadius: 7, fontSize: 12, fontWeight: 500, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", gap: 5 }}
                      onMouseEnter={(e) => { e.currentTarget.style.borderColor = "var(--cal2-primary)"; }}
              ...
              {error && (
                <div style={{ padding: "8px 12px", background: "var(--cal2-danger-bg)", border: "1px solid var(--cal2-danger-border)", borderRadius: 7, fontSize: 12.5, color: "var(--cal2-danger-text)", textWrap: "balance" }}>
                  {error}
                </div>
              )}
            </>
...
function DayRow({
  ...
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "6px 4px", borderBottom: "1px solid var(--cal2-border-faint)" }}>
      {/* Toggle */}
      <button
        onClick={() => onToggleEnabled(!sched.enabled)}
        style={{ position: "relative", width: 32, height: 18, background: sched.enabled ? "var(--cal2-primary)" : "var(--cal2-border)", borderRadius: 999, flexShrink: 0, transition: "background 140ms", cursor: "pointer", border: "none", padding: 0, marginTop: 6 }}
      >
        <span style={{ position: "absolute", top: 2, left: sched.enabled ? 16 : 2, width: 14, height: 14, background: "var(--cal2-toggle-thumb)", borderRadius: "50%", transition: "left 140ms" }} />
      </button>

      {/* Day label */}
      <span style={{ fontSize: 12.5, color: sched.enabled ? "var(--cal2-text)" : "var(--cal2-text-faint)", fontWeight: 500, width: 84, flexShrink: 0, paddingTop: 7 }}>
        {label}
      </span>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6, minWidth: 0, opacity: sched.enabled ? 1 : 0.45 }}>
        {sched.ranges.length === 0 && sched.enabled && (
          <div style={{ fontSize: 11.5, color: "var(--cal2-text-faint)", padding: "4px 0" }}>
          ...
          <div key={idx} style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {sched.enabled && (
              <button
                ...
                style={{ width: 22, height: 22, border: "none", background: "transparent", borderRadius: 5, color: "var(--cal2-text-faint)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
                onMouseEnter={(e) => { e.currentTarget.style.background = "var(--cal2-danger-hover-bg)"; e.currentTarget.style.color = "var(--cal2-closed-border)"; }}
                onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "var(--cal2-text-faint)"; }}
              >
        ...
        {sched.enabled && (
          <div style={{ display: "flex", gap: 12, alignItems: "center", paddingTop: 2 }}>
            <button
              onClick={onAddRange}
              style={{ background: "transparent", color: "var(--cal2-primary)", border: "none", fontSize: 11.5, fontWeight: 500, cursor: "pointer", fontFamily: "inherit", display: "inline-flex", alignItems: "center", gap: 4, padding: 0 }}
            >
            ...
            <div style={{ position: "relative" }}>
              <button
                onClick={onCopyClick}
                style={{ background: "transparent", color: "var(--cal2-text-muted)", border: "none", fontSize: 11.5, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}
              >
                Copier vers…
              </button>
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          borderRadius: 8,
          boxShadow: "var(--cal2-shadow-popover)",
          padding: 10,
        }}
      >
        <div style={{ fontSize: 11, fontWeight: 600, color: "var(--cal2-text-soft)", marginBottom: 8 }}>
          Appliquer à
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          {candidates.map((d) => (
            <label
              key={d.val}
              style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "var(--cal2-text)", cursor: "pointer", padding: "3px 4px" }}
            >
              <input
                type="checkbox"
                checked={picked.has(d.val)}
                onChange={() => toggle(d.val)}
                style={{ accentColor: "var(--cal2-primary)" }}
              />
              {d.short}
            </label>
          ))}
        ...
        <div style={{ marginTop: 10, display: "flex", gap: 6, justifyContent: "flex-end" }}>
          <button
            onClick={onClose}
            style={{ padding: "5px 10px", background: "transparent", color: "var(--cal2-text-muted)", border: "1px solid var(--cal2-border-soft)", borderRadius: 6, fontSize: 11.5, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}
          >
            Annuler
          </button>
          <button
            ...
            style={{ padding: "5px 10px", background: picked.size > 0 ? "var(--cal2-primary)" : "var(--cal2-border)", color: picked.size > 0 ? "var(--cal2-toggle-thumb)" : "var(--cal2-text-faint)", border: "none", borderRadius: 6, fontSize: 11.5, fontWeight: 500, cursor: picked.size > 0 ? "pointer" : "default", fontFamily: "inherit" }}
          >
            Appliquer
          </button>
        </div>
...
function ExceptionRow({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6, padding: 10, border: "1px solid var(--cal2-border-faint)", borderRadius: 8, background: "var(--cal2-surface)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <button
          ...
          style={{
            padding: "4px 10px",
            border: "1px solid",
            borderColor: closed ? "var(--cal2-closed-border)" : "var(--cal2-border-soft)",
            borderRadius: 6,
            background: closed ? "var(--cal2-closed-bg)" : "var(--cal2-surface)",
            color: closed ? "var(--cal2-closed-text)" : "var(--cal2-text-soft)",
            fontSize: 11,
            fontWeight: 500,
            cursor: "pointer",
            fontFamily: "inherit",
        ...
        <button
          ...
          style={{ width: 24, height: 24, border: "none", background: "transparent", borderRadius: 5, color: "var(--cal2-text-faint)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}
          onMouseEnter={(e) => { e.currentTarget.style.background = "var(--cal2-danger-hover-bg)"; e.currentTarget.style.color = "var(--cal2-closed-border)"; }}
          onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "var(--cal2-text-faint)"; }}
        >
      ...
      {!closed && (
        <div style={{ display: "flex", flexDirection: "column", gap: 4, paddingLeft: 4 }}>
          {ranges.map((r, idx) => (
            <div key={idx} style={{ display: "flex", alignItems: "center", gap: 6 }}>
              ...
              <button
                ...
                style={{ width: 22, height: 22, border: "none", background: "transparent", borderRadius: 5, color: "var(--cal2-text-faint)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
              >
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><line x1="18" y1="6" x2="6" y2="18" /><line x1="6" y1="6" x2="18" y2="18" /></svg>
              </button>
            </div>
          ...
          <button
            onClick={addRange}
            style={{ alignSelf: "flex-start", background: "transparent", color: "var(--cal2-primary)", border: "none", fontSize: 11, fontWeight: 500, cursor: "pointer", fontFamily: "inherit", padding: 0 }}
          >
            + Ajouter une plage
          </button>
        </div>
  ...
  return (
    ...
    >
      ...
      <select
        ...
        className="cal2-notice-select"
        style={{
          padding: "8px 28px 8px 12px",
          border: "none",
          backgroundColor: "var(--cal2-surface)",
          fontSize: 12.5,
          color: "var(--cal2-text)",
          fontFamily: "inherit",
          outline: "none",

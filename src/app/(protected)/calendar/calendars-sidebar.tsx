...
const CAL_COLORS: { color: string; accent: string }[] = [
  ...
  { color: "var(--cal2-text-muted)", accent: "var(--cal2-surface-2)" },
];
  ...
  return (
    <aside style={{ width: 240, flexShrink: 0, background: "var(--cal2-canvas-soft)", borderRight: "1px solid var(--cal2-border-faint)", display: "flex", flexDirection: "column", padding: "18px 14px 14px", overflowY: "auto", height: "100%" }}>
      ...
      <button
        ...
        style={{ marginTop: 12, padding: "9px 12px", background: "var(--cal2-surface)", border: "1px dashed var(--cal2-border)", borderRadius: 8, color: "var(--cal2-text-soft)", fontSize: 12.5, fontWeight: 500, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, cursor: "pointer", fontFamily: "inherit", transition: "all 120ms" }}
        onMouseEnter={(e) => { e.currentTarget.style.borderColor = "#0052D9"; e.currentTarget.style.color = "#0052D9"; e.currentTarget.style.background = "var(--cal2-blue-tint)"; }}
        onMouseLeave={(e) => { e.currentTarget.style.borderColor = "var(--cal2-border)"; e.currentTarget.style.color = "var(--cal2-text-soft)"; e.currentTarget.style.background = "var(--cal2-surface)"; }}
      >
      ...
      {canEdit && hovered && (
        <div style={{ display: "flex", gap: 2, flexShrink: 0 }}>
          <button
            ...
            onMouseEnter={(e) => (e.currentTarget.style.background = "var(--cal2-surface-2)")}
...
function CalModal({ title, initial, onClose, onSave }: CalModalProps) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.35)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--cal2-surface)", borderRadius: 12, boxShadow: "0 24px 60px rgba(15,23,42,0.2)", width: "100%", maxWidth: 340, padding: "22px 22px 18px" }}>
        <h3 style={{ fontSize: 15, fontWeight: 600, color: "var(--cal2-text)", marginBottom: 18 }}>{title}</h3>

        <div style={{ marginBottom: 16 }}>
          <label style={{ fontSize: 11.5, fontWeight: 500, color: "var(--cal2-text-soft)", display: "block", marginBottom: 6 }}>Nom</label>
          <input
            ...
            style={{ width: "100%", padding: "8px 11px", border: "1px solid var(--cal2-border-soft)", borderRadius: 7, fontSize: 13, fontFamily: "inherit", outline: "none", boxSizing: "border-box" }}
          />
        </div>

        <div style={{ marginBottom: 22 }}>
          <label style={{ fontSize: 11.5, fontWeight: 500, color: "var(--cal2-text-soft)", display: "block", marginBottom: 8 }}>Couleur</label>
        ...
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <button onClick={onClose} style={{ padding: "7px 14px", background: "transparent", color: "var(--cal2-text-muted)", border: "1px solid var(--cal2-border-soft)", borderRadius: 7, fontSize: 12.5, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>Annuler</button>
          <button
            disabled={!name.trim()}
            onClick={() => name.trim() && onSave({ name: name.trim(), ...CAL_COLORS[colorIdx] })}
            style={{ padding: "7px 14px", background: name.trim() ? "#0052D9" : "var(--cal2-border)", color: "var(--cal2-surface)", border: "none", borderRadius: 7, fontSize: 12.5, fontWeight: 500, cursor: name.trim() ? "pointer" : "default", fontFamily: "inherit" }}
          >
            Enregistrer
          </button>
        </div>
...
function DeleteConfirm({ name, onConfirm, onCancel }: { name: string; onConfirm: () => void; onCancel: () => void }) {
  return (
    <div onClick={onCancel} style={{ position: "fixed", inset: 0, background: "rgba(15,23,42,0.35)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: "var(--cal2-surface)", borderRadius: 12, boxShadow: "0 24px 60px rgba(15,23,42,0.2)", width: "100%", maxWidth: 320, padding: "24px 24px 20px" }}>
        <h3 style={{ fontSize: 15, fontWeight: 600, color: "var(--cal2-text)", marginBottom: 8 }}>Supprimer cet agenda ?</h3>
        <p style={{ fontSize: 13, color: "var(--cal2-text-muted)", marginBottom: 22, lineHeight: 1.5 }}>
          L&apos;agenda <strong>{name}</strong> sera supprimé. Les événements liés resteront dans votre calendrier.
        </p>
        <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
          <button onClick={onCancel} style={{ padding: "7px 14px", background: "transparent", color: "var(--cal2-text-muted)", border: "1px solid var(--cal2-border-soft)", borderRadius: 7, fontSize: 12.5, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>Annuler</button>
          <button onClick={onConfirm} style={{ padding: "7px 14px", background: "#EF4444", color: "var(--cal2-surface)", border: "none", borderRadius: 7, fontSize: 12.5, fontWeight: 500, cursor: "pointer", fontFamily: "inherit" }}>Supprimer</button>
        </div>
      </div>
    </div>
  );

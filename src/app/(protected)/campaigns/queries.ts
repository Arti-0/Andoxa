...
function mapJob(
    ...
    const fallbackName =
        (meta as { name?: string }).name?.trim() ||
        campaignLabel(configFromJobType(job.type));
...
async function fetchJobStats(
    workspaceId: string
): Promise<Map<string, JobStatsRow>> {
    try {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            const [data, stats] = await Promise.all([
                getJson<{ items: ApiCampaignJob[] }>(
                    '/api/campaigns/jobs?pageSize=100'
                ),
                workspaceId
                    ? fetchJobStats(workspaceId)
                    : Promise.resolve(new Map<string, JobStatsRow>()),
            ]);
...
export function useCampaignJobDetail(jobId: string | undefined) {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            const raw =
                json.data ??
                (json as unknown as ApiCampaignJob & {
                    prospects: CampaignJobProspectRow[];
                });
            ...
            return {
                ...
                attachment: readCampaignAttachment(
                    jobRow.metadata
...
function mapSession(
    ...
    const activeUser = s.active_user_id
        ? members.get(s.active_user_id)
        : undefined;
    return {
        id: s.id,
        kind: 'session',
        ...
        liveStatus,
        activeUserName:
            liveStatus === 'in_progress' ? (activeUser?.name ?? null) : null,
        date: s.created_at,
...
export async function patchCallSessionProspect(
    ...
    return patchJson(
        `/api/call-sessions/${sessionId}/prospects/${prospectId}`,
        body
    );
}

export async function postCallSessionNote(
    sessionId: string,
    prospect_id: string,
    content: string
) {
    return postJson(`/api/call-sessions/${sessionId}/notes`, {
        prospect_id,
        content,
    });
}
...
export function useUpdateJobStatus() {
    ...
    return useMutation({
        ...
        onMutate: async ({ id, status }) => {
            ...
            const prev = qc.getQueryData<Campaign[]>([
                'campaigns',
                'jobs',
                workspaceId,
            ]);
            qc.setQueryData<Campaign[]>(
                ['campaigns', 'jobs', workspaceId],
                (old) =>
                    (old ?? []).map((c) =>
                        c.id === id
                            ? {
                                  ...c,
                                  status: statusToDesign(
                                      status as CampaignJobStatus
                                  ),
                              }
                            : c
                    )
        ...
        onError: (err, _vars, ctx) => {
            if (ctx?.prev)
                qc.setQueryData(['campaigns', 'jobs', workspaceId], ctx.prev);
            toast.error(
...
export function useLaunchJob() {
    ...
    return useMutation({
        ...
        onSuccess: (_d, id) => {
            ...
            void qc.invalidateQueries({
                queryKey: ['campaigns', 'job', workspaceId, id],
            });
    ...
    return useMutation({
        mutationFn: async (id: string) => {
            const env = (await postJson(
                `/api/campaigns/jobs/${id}/retry-errors`
            )) as {
        ...
        onError: (err) => {
            toast.error(
                err instanceof Error ? err.message : 'Réessai impossible'
            );
        },
    });
}
...
export function useCampaignJobsBulk() {
    ...
    return useMutation({
        ...
        onError: (err) => {
            toast.error(
                err instanceof Error ? err.message : 'Action groupée impossible'
            );
        },
    });
}
...
export function useDeleteSession() {
    ...
    return useMutation({
        ...
        onMutate: async (id) => {
            await qc.cancelQueries({
                queryKey: ['campaigns', 'sessions', workspaceId],
            });
            const prev = qc.getQueryData<CallSession[]>([
                'campaigns',
                'sessions',
                workspaceId,
            ]);
            qc.setQueryData<CallSession[]>(
                ['campaigns', 'sessions', workspaceId],
        ...
        onSettled: (_d, _e, id) => {
            ...
            if (id) {
                void qc.invalidateQueries({
                    queryKey: ['campaigns', 'call-session', workspaceId, id],
                });
            }
        },
    });

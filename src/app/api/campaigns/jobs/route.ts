    ...
    async (req, ctx) => {
        ...
        if (!body.type || !allowedTypes.includes(body.type)) {
            throw Errors.validation({
                type: "Must be 'invite', 'invite_with_note', 'invite_then_message', 'contact', or 'whatsapp'",
            });
        }
        ...
        if (
            ...
            if (
                typeof path !== 'string' ||
                !path.startsWith(`${ctx.workspaceId}/`)
            ) {
                throw Errors.validation({
            ...
            if (
                typeof size === 'number' &&
                size > CAMPAIGN_ATTACHMENT_MAX_BYTES
            ) {
                throw Errors.validation({
                    attachment:
                        'La pièce jointe dépasse la taille maximale (20 Mo).',
                });
            }
            attachmentMeta = {
                path,
        ...
        const rawIds = [...new Set([...(body.prospect_ids ?? []), ...bddIds])];

        if (rawIds.length === 0 && body.launch_now) {
            throw Errors.validation({
                prospect_ids:
                    'At least one prospect is required to launch immediately',
            });
        }
        ...
        if (rawIds.length === 0) {
            ...
            if (overrides && Object.keys(overrides).length > 0)
                metadata.message_overrides = overrides;
            ...
            if (
                ...
                body.invite_note_template?.trim()
            ) {
                metadata.invite_note_template =
                    body.invite_note_template.trim();
            }
        ...
        if (insertError)
            throw Errors.internal('Failed to add prospects to job');

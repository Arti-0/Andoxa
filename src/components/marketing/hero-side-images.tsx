...
const CARDS: Card[] = [
  { src: ANDOXA_PAGE.messagerie, side: "left",  top: "6%",  offset: "-16%", width: 450, rotate: -9, delay: 0.55, bob: 8 },
  { src: ANDOXA_PAGE.calendrier, side: "left",  top: "44%", offset: "-22%", width: 525, rotate: -3, delay: 0.7,  bob: 6 },
  { src: ANDOXA_PAGE.workflows,  side: "left",  top: "80%", offset: "-14%", width: 400, rotate: 7,  delay: 0.85, bob: 10 },
  { src: ANDOXA_PAGE.crm,        side: "right", top: "8%",  offset: "-18%", width: 525, rotate: 7,  delay: 0.6,  bob: 7 },
  { src: ANDOXA_PAGE.dashboard,  side: "right", top: "48%", offset: "-14%", width: 450, rotate: -4, delay: 0.75, bob: 9 },
  { src: ANDOXA_PAGE.campagnes,  side: "right", top: "82%", offset: "-20%", width: 475, rotate: 5,  delay: 0.9,  bob: 6 },
];
...
function FloatingCard({ card, reduce }: { card: Card; reduce: boolean }) {
  ...
  return (
    ...
    >
      ...
      >
        <BrowserCard src={card.src} />
      </motion.div>
    </motion.div>
  );
}

function BrowserCard({ src }: { src: string }) {
  return (
    <div className="overflow-hidden rounded-xl border border-[var(--border)] bg-card shadow-2xl shadow-[0_60px_140px_-25px_rgba(0,82,217,0.50),_0_30px_70px_-15px_rgba(0,82,217,0.28),_0_10px_30px_-8px_rgba(0,0,0,0.18)]">
      ...
      <EmbeddedPage src={src} title="" className="block w-full" />
    </div>
  );
}

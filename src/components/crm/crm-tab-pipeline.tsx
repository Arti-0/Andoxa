...
export function PipelineTab({
  ...
  const [collapsed, setCollapsed] = useState<Set<ProspectStatus>>(() => {
    ...
    return new Set(
      (PIPELINE_ORDER as ProspectStatus[]).filter((s) => s !== initialFunnel),
    );
  });
  ...
  const counts = useMemo(() => {
    ...
    for (const p of items) {
      if (p.status && (m as Record<string, number>)[p.status] !== undefined)
  ...
  const onFunnelClick = (id: ProspectStatus) => {
    if (funnelFilter === id) {
      ...
      setCollapsed(
        new Set(PIPELINE_ORDER.filter((s) => s !== id) as ProspectStatus[]),
      );
    }
  };
          ...
          return (
            ...
            >
              ...
              <div className="flex items-baseline justify-between gap-1.5">
                ...
                >
                  {dir === "up" && <ArrowUp className="h-2 w-2" />}
                  {dir === "down" && <ArrowDown className="h-2 w-2" />}
                  {dir !== "flat" ? `${delta > 0 ? "+" : ""}${delta}` : "—"}
                </span>
              </div>
            </button>
          );
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {[
                    ...
                    const active = pipelineSourceFilter.includes(opt.value);
                    return (
                      <button
                        key={opt.value}
                        onClick={() =>
                          setPipelineSourceFilter((prev) =>
          ...
          {(PIPELINE_ORDER as ProspectStatus[]).map((stageId) => {
            ...
            return (
              <PipelineSection
                ...
                cycleDays={funnelByStatus.get(stageId)?.cycle ?? null}
        ...
        <div className="flex min-w-0 items-center gap-2.5">
          ...
          <div className="min-w-0">
            ...
            <div className="mt-0.5 truncate text-[11.5px] text-muted-foreground">
              {p.company ? (
                <strong className="font-medium">{p.company}</strong>
              ) : (
                <span className="text-muted-foreground/60">Sans société</span>
              )}
        ...
        <div data-stop className="inline-flex items-center justify-end">
          {(isHovered || isMenuOpen) && (
            <>
              ...
              <GhostBtn title="RDV" onClick={() => onOpenProspect(p)}>
